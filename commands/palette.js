	...
	https.get('https://lospec.com/palette-list/' + paletteName, (res) => {
		...
		res.on('end', () => {
			...
			if (match) {
				...
				console.log('https://lospec.com/palette-list/' + paletteName + '-32x.png');
				console.log(embed);
...
function unabbreviate(str) {
	...
	switch (str) {
		case (str.match(/^en?d(es)?ga?-*[0-9]+-*x?$/) || {}).input:
		...
		case (str.match(/^d(awn)?-*b(ringer)?-*[0-9]+$/i) || {}).input:
			return 'DawnBringer-' + str.slice(str.indexOf(str.match(/[0-9]+$/)[0]), str.length);

		case (str.match(/^andrew-*ken?sler-*[0-9]+$/) || {}).input:
